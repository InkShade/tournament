...
const TournamentGrid: React.FC<GridProps> = ({ matches }) => {
      ...
      matchElements.forEach((el) => {
        ...
        if (match && match.tournamentRoundText) {
          const currentRound = parseInt(
            match.tournamentRoundText.split(" ")[1]
          );
          const nextRound = currentRound + 1;

          const nextRoundMatches = clientMatches.filter(
            (m) => parseInt(m.tournamentRoundText.split(" ")[1]) === nextRound
          );
          const currentRoundMatches = clientMatches.filter(
            (m) =>
              parseInt(m.tournamentRoundText.split(" ")[1]) === currentRound
          );
            ...
            if (nextMatchEl) {
              ...
              if (matchIndexInCurrentRound % 2 === 0) {
                const nextMatchEl2 = bracketRef.current!.querySelector(
                  `[data-match-id="${
                    currentRoundMatches[matchIndexInCurrentRound + 1].id
                  }"]`
                );
  ...
  return (
    <div
      ref={bracketRef}
      className="relative flex overflow-x-auto p-6 w-full mx-auto whitespace-nowrap"
    >
      <svg
      ...
      {Array.from({ length: rounds }, (_, roundIndex) => {
        const roundMatches = clientMatches.filter(
          (match) => match.tournamentRoundText === `Round ${roundIndex + 1}`
        );
        const roundName = getRoundName(roundIndex, roundMatches.length);
        return (
          <div
            key={roundIndex}
            className="flex flex-col justify-around mr-8 relative"
          >
                  ...
                  >
                    <span
                      className={`text-sm font-medium ${
                        participant.isWinner ? "text-red-600" : "text-gray-700"
                      }`}
                    >
                      {participant.name}
                    </span>
                    <span
                      className={`text-xs text-gray-500 ${
                        participant.isWinner ? "text-red-600" : "text-gray-500"
                      }`}
                    >
